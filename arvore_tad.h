...
int EstaNaArvore(ArvoreBinaria X, int ch, int num) {
    if (X == NULL) {
        ...
        cout << "O valor " << ch << " está na árvore." << endl;            
            ...
            if (ch>X->info) {
                return EstaNaArvore(X->dir, ch, num);
            }
            else {
    
                return EstaNaArvore(X->esq, ch, num);
            }
        }
    }
}
...
void jogar(ArvoreBinaria x) {
    ArvoreBinaria atual = x;
    int info;
    while (atual != NULL) {
        cout << "O número é " << atual->info << "? ";
        cin >> info;
        if (info == 1) {
            cout << endl;
            cout << "O gênio acertou!!!!" << endl;
            cout << endl;
            return;
        }
        else {
            cout << "O número é maior ou menor que " << atual->info << "? " ;
            cin >> info;
            if (info == 1) {
                atual = atual->dir;
            }
